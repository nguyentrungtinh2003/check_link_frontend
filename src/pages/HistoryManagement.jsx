          ...
          <Table striped bordered hover responsive>
            ...
            <tbody>
              {histories?.map((h, index) => {
                ...
                const isDanger =
                  vtData && vtData.stats && vtData.stats.malicious > 0; // nếu có malicious > 0

                return (
                  <tr key={h.id}>
                    <td>{index + 1}</td>
                    <td title={h.urlCheck}>{h.urlCheck}</td>

                    {/* Google Safe Browsing */}
                    <td style={cellStyle} title={h.googleSafeBrowsing}>
                      {h.googleSafeBrowsing.includes("An toàn") ? (
                        <Badge className="border rounded-circle" bg="success">
                          <FaCheck />
                        </Badge>
                      ) : (
                        <Badge className="border rounded-circle" bg="danger">
                          <FaExclamationTriangle />
                        </Badge>
                      )}
                    </td>
                    ...
                    <td
                      style={cellStyle}
                      title={isDanger ? "Nguy hiểm" : "An toàn"}
                    >
                      {isDanger ? (
                        <Badge className="border rounded-circle" bg="danger">
                          <FaExclamationTriangle />
